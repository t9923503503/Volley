...
function setCustomRounds(val) {
  ...
  customRounds = val;
  // Expand score arrays if needed
  for (let ci = 0; ci < 4; ci++) {
    for (let mi = 0; mi < ppc; mi++) {
      while (scores[ci][mi].length < val) scores[ci][mi].push(null);
    }
  }
  saveState();
  const el = document.getElementById('roster-rounds-val');
...
function setCustomTours(val) {
  ...
  saveState();
  const el = document.getElementById('roster-tours-val');
  if (el) el.textContent = customTours;
}

function setFixedPair(ci, mi, wi) {
  if (!fixedPairs[ci]) fixedPairs[ci] = [];
  fixedPairs[ci][mi] = +wi;
  saveState();
  // Update court view if visible
  const s = document.getElementById(`screen-${ci}`);
  if (s) s.innerHTML = renderCourt(ci);
}

function _renderFixedPairsEditor() {
  let html = `<div class="settings-card"><div class="sc-title">👫 Назначить пары</div>`;
  for (let ci = 0; ci < nc; ci++) {
    const meta = COURT_META[ci];
    const men   = ALL_COURTS[ci].men.slice(0, ppc);
    const women = ALL_COURTS[ci].women.slice(0, ppc);
    html += `<div style="margin-bottom:12px">
      <div style="font-size:11px;font-weight:700;color:${meta.color};margin-bottom:6px">${meta.name}</div>`;
    for (let mi = 0; mi < ppc; mi++) {
      const curWi = (fixedPairs[ci] && fixedPairs[ci][mi] != null) ? fixedPairs[ci][mi] : mi % ppc;
      html += `<div class="sc-row" style="gap:8px;margin-bottom:4px">
        <span style="min-width:90px;font-size:12px">🏋️ ${esc(men[mi] || `М${mi+1}`)}</span>
        <select class="arch-form-sel" style="flex:1;font-size:12px" onchange="setFixedPair(${ci},${mi},this.value)">
          ${women.map((w, wi) => `<option value="${wi}"${wi===curWi?' selected':''}>${esc(w || `Ж${wi+1}`)}</option>`).join('')}
        </select>
      </div>`;
    }
    html += `</div>`;
...
async function applySettings() {
  ...
  ppc = _ppc; nc = _nc;
  // Clamp customRounds to new ppc if in rotation mode
  if (pairMode === 'rotation' && customRounds > ppc) customRounds = ppc;
  // Re-init fixedPairs for new ppc
  fixedPairs = Array.from({length:4}, (_, ci) => Array.from({length:ppc}, (_, mi) => mi % ppc));
...
function renderRoster() {
  ...
  <!-- Режим пар и кол-во раундов/туров -->
  <div class="settings-card">
    <div class="sc-title">👫 Режим пар и раунды</div>
    <div class="sc-row">
      <span class="sc-lbl">Режим:</span>
      <div class="seg">
        <button class="seg-btn${pairMode==='rotation'?' on':''}" onclick="setPairMode('rotation')">🔄 Ротация</button>
        <button class="seg-btn${pairMode==='fixed'?' on':''}" onclick="setPairMode('fixed')">👫 Фикс. пара</button>
      </div>
    </div>
    <div class="sc-row">
      <span class="sc-lbl">Раундов:</span>
      <div style="display:flex;align-items:center;gap:6px">
        <button class="timer-custom-btn" onclick="setCustomRounds(customRounds-1)">−</button>
        <div class="timer-custom-val active" id="roster-rounds-val">${customRounds}</div>
        <button class="timer-custom-btn" onclick="setCustomRounds(customRounds+1)">+</button>
      </div>
    </div>
    <div class="sc-row">
      <span class="sc-lbl">Туров:</span>
      <div style="display:flex;align-items:center;gap:6px">
        <button class="timer-custom-btn" onclick="setCustomTours(customTours-1)">−</button>
        <div class="timer-custom-val active" id="roster-tours-val">${customTours}</div>
        <button class="timer-custom-btn" onclick="setCustomTours(customTours+1)">+</button>
      </div>
    </div>
    <div class="sc-info">Раунды: 1–10 · Туры: 1–5</div>
  </div>

  ${pairMode === 'fixed' ? _renderFixedPairsEditor() : ''}
