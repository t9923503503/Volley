...
// Balanced display order: for n=5 uses a perfect design where every pair
// of players appears adjacent exactly 2 times across all 5 rounds.
// For other sizes falls back to a step-ceil(n/2) cyclic permutation.
function getDisplayOrder(n, ri) {
  if (n === 5) {
    const T = [
      [0,1,2,3,4],
      [1,3,0,4,2],
  ...
  const step = n <= 2 ? 1 : Math.ceil(n / 2);
  return Array.from({length: n}, (_, i) => (i * step + ri) % n);
}
...
function saveState() {
  try {
    ...
    localStorage.setItem('kotc3_cfg',        JSON.stringify({ ppc, nc, pairMode, fixedPairs, customRounds, customTours }));
...
function loadState() {
  try {
    ...
    if (cfg) {
      ...
      if (p.pairMode === 'fixed' || p.pairMode === 'rotation') pairMode = p.pairMode;
      if (Array.isArray(p.fixedPairs))       fixedPairs = p.fixedPairs;
      if (p.customRounds >= 1 && p.customRounds <= 10) customRounds = +p.customRounds;
      if (p.customTours  >= 1 && p.customTours  <= 5)  customTours  = +p.customTours;
    }
...
function buildNav() {
  ...
  [
    { label:'🏠',   tab:'home'    },
    { label:'👤',   tab:'players' },
    { label:'ТУР 1', tab:'svod'    },
    { label:'ТУР 2', tab:'stats'   },
    { label:'👥',   tab:'rating'  },
    { label:'⚙️',   tab:'roster'  },
  ].forEach(({label,tab}) => {
