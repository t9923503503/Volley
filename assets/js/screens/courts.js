...
// ── Партнёр: ротация или фиксированная пара ─────────────────
...
function renderCourt(ci) {
  ...
  for (let ri = 0; ri < rounds; ri++) {
    ...
    html += `<div class="round-lbl">Раунд ${ri+1}${customTours > 1 ? ` · Туров: ${customTours}` : ''}</div>`;
    for (let i = 0; i < ppc; i++) {
      const mi  = pairMode === 'fixed' ? i : getDisplayOrder(ppc, ri)[i];
      const wi  = getPartnerW(ci, mi, ri);
...
function renderDivCourt(key, svodM, svodW) {
  ...
  for (let ri = 0; ri < Nd; ri++) {
    ...
    for (let i = 0; i < Nd; i++) {
      const mi  = getDisplayOrder(Nd, ri)[i];
