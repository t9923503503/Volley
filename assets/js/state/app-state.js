...
// ── Pair mode & custom rounds/tours ─────────────────────────
// 'rotation' = KotC rotating partners (default)
// 'fixed'    = each man has a fixed partner for the whole tournament
let pairMode = 'rotation';
// fixedPairs[ci][mi] = wi index of partner woman (0..ppc-1)
let fixedPairs = Array.from({length:4}, (_, ci) => Array.from({length:5}, (_, mi) => mi % 5));
// customRounds: how many rounds to play per court (default = ppc, i.e. same as before)
let customRounds = ppc;
// customTours: how many tours (sets of all rounds) to play (1-5)
let customTours = 1;
